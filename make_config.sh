...
for n in "${sizes[@]}"; do
    ...
    echo '[[jobs]]'
    echo "name = \"matmul-batch-parallel-bf16f32-${batch_size}x${n}x${n}x${n}\""
    echo "size = $n"
    echo "batch_size = \"$batch_size\""
    echo "gflops = $gflops_value"
    echo "backend_name = \"morello\""
...
for batch_size in $(seq 2 "$PHYSICAL_CORES" | sed -e "/^$(( PHYSICAL_CORES / 2 ))$/d" -e "/^$PHYSICAL_CORES$/d"); do
    ...
    echo '[[jobs]]'
    echo "name = \"matmul-batch-parallel-bf16f32-${batch_size}x2048x2048x2048\""
    echo "size = 2048"
    echo "batch_size = \"$batch_size\""
    echo "gflops = $gflops_value"
    echo "backend_name = \"morello\""
